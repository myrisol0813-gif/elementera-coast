...
const FRESHNESS = new Set(['live', 'today', 'recent', 'old_but_relevant', 'archived', 'deprecated']);
...
function intentFromInput(input) {
  const text = clip(input, 1200).replace(/\s+/g, ' ');
  if (!text) return '承接当前对话。';
  const first = text.split(/[。！？!?\n]/)[0];
  return clip(first || text, 220);
}
...
export function buildContextManifest({
  ...
  return contextBlock({
    ...
    trace: { surface, block_keys: usable.map((block) => block.key), contradiction_count: contradictionCount },
  });
}
