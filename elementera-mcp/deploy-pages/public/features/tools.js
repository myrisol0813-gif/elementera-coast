...
export function createTools({ storage, router, toast, memory }) {
  ...
  function choiceRow(name, title, note = '') {
    ...
    const current = name === 'streamingEnabled' ? Boolean(settings[name]) : settings[name];
  ...
  function handleInput(name, target) {
    ...
    storage.update((state) => {
      if (target.name === 'streamingEnabled') state.runControl.streamingEnabled = target.value === 'true';
