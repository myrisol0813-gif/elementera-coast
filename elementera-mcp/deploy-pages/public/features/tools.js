...
export function createTools({ storage, router, toast, memory }) {
  ...
  const choices = Object.freeze({
    ...
    calendarInjection: [['off', '关闭'], ['today_only', '只看今天'], ['only_when_events', '有内容时'], ['manual', '手动']],
    worldbookEnabled: [[false, '关闭'], [true, '开启']],
    memoryFacetsEnabled: [[false, '关闭'], [true, '开启']],
    contextDebug: [[false, '关闭'], [true, '开启']],
    ambientTime: [[false, '关闭'], [true, '开启']],
    ambientCalendar: [[false, '关闭'], [true, '开启']],
    ambientTools: [[false, '关闭'], [true, '开启']],
    ambientRoom: [[false, '关闭'], [true, '开启']],
    ambientModel: [[false, '关闭'], [true, '开启']],
  });

  function choiceRow(name, title, note = '') {
    const settings = storage.read().runControl;
    const current = ['streamingEnabled', 'worldbookEnabled', 'memoryFacetsEnabled', 'contextDebug', 'ambientTime', 'ambientCalendar', 'ambientTools', 'ambientRoom', 'ambientModel'].includes(name) ? Boolean(settings[name]) : settings[name];
  ...
  function runControlView() {
    return {
      ...
      body: `<p class="feature-note">这些设置保存在统一的本地状态中，并直接供主聊天请求读取。</p>
        ${group('上下文预算', choiceRow('recentTurns', '最近上下文轮数') + choiceRow('contextBudget', '上下文预算（估算 token）', '按最近轮数与估算 token 上限裁剪；中文约一字一 token，英文约四字符一 token。'))}
        ...
        ${group('Context Manifest', '<button class="feature-row" type="button" data-action="context:modes"><span><strong>当前情境卡</strong><small>普通 / 施工 / 代码 / 创作 / 陪伴…</small></span><span>›</span></button>' + choiceRow('calendarInjection', '日历注入') + choiceRow('worldbookEnabled', '海岸词典') + choiceRow('memoryFacetsEnabled', '记忆球情境面') + choiceRow('contextDebug', 'Context Inspector 数据') + numberRow('worldbookLimit', '词典最大注入条数', 0, 6, 1, '同轮最多 6 条。') + numberRow('memoryLimit', '记忆最大注入条数', 0, 12, 1, '仍受各记忆池上限与总预算约束。'))}
        ${group('海岸环境包', choiceRow('ambientTime', '本地时间') + choiceRow('ambientCalendar', '今日日历') + choiceRow('ambientTools', '工具简表') + choiceRow('ambientRoom', '当前房间') + choiceRow('ambientModel', '当前模型'))}
  ...
  function handleInput(name, target) {
    ...
    storage.update((state) => {
      if (['streamingEnabled', 'worldbookEnabled', 'memoryFacetsEnabled', 'contextDebug', 'ambientTime', 'ambientCalendar', 'ambientTools', 'ambientRoom', 'ambientModel'].includes(target.name)) state.runControl[target.name] = target.value === 'true';
